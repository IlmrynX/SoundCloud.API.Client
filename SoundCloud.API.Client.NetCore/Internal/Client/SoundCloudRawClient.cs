...
namespace SoundCloud.API.Client.Internal.Client
{
    internal class SoundCloudRawClient : ISoundCloudRawClient
    {
        ...
        public async Task<T> Request<T>(string apiPrefix, string command, HttpMethod method, Dictionary<string, object> parameters = null, byte[] body = null, string responseType = "json", Domain domain = Domain.Api)
            where T : class 
        {
            var response = await GetResponse(domain, apiPrefix, command, method, parameters, body, responseType);
            return typeof(T) == typeof(string) ? response as T : serializer.Deserialize<T>(response);
        }

        public async Task Request(string apiPrefix, string command, HttpMethod method, Dictionary<string, object> parameters = null, byte[] body = null, Domain domain = Domain.Api)
        {
            await GetResponse(domain, apiPrefix, command, method, parameters, body, string.Empty);
        }
        ...
        public async Task<T> Upload<T>(string prefix, string command, Dictionary<string, object> parameters, string responseType = "json", Domain domain = Domain.Api, params File[] files)
        {
            var uriBuilder = CreateUriBuilder(domain, prefix, command, responseType);
            var response = await webGateway.Upload(uriBuilder, parameters, files);
            return serializer.Deserialize<T>(response);
        }

        public async Task<Stream> RequestStream(string apiPrefix, string command, HttpMethod method, Dictionary<string, object> parameters = null, byte[] body = null, Domain domain = Domain.Api)
        {
            var token = GetToken();
            var uriBuilder = CreateUriBuilder(domain, apiPrefix, command, string.Empty);
            return await webGateway.RequestStream(uriBuilder, method, parameters, body, token);
        }

        private async Task<string> GetResponse(Domain domain, string prefix, string command, HttpMethod method, Dictionary<string, object> parameters, byte[] body, string responseType)
        {
            var token = GetToken();
            var uriBuilder = CreateUriBuilder(domain, prefix, command, responseType);
            var response = await webGateway.Request(uriBuilder, method, parameters, body, token);
            return response;
        }
