...
namespace SoundCloud.API.Client.Subresources
{
    public class TracksApi : ITracksApi
    {
        ...
        public async Task<SCTrack> UploadTrack(Stream trackFileStream, string title, string description, SCSharing sharing, Stream artworkFileStream)
        {
            ...
            var uploadedTrack = await soundCloudRawClient.Upload<Track>(prefix, string.Empty, parameters, files: files.ToArray());
        ...
        public ITracksSearcher BeginSearch(SCFilter filter, SCApiVersion version)
        {
            ...
            Func<Dictionary<string, object>, Task<SCTrack[]>> publicTracksSearch = async parameters => (await internalTracksSearch(parameters)).Select(trackConverter.Convert).ToArray();
