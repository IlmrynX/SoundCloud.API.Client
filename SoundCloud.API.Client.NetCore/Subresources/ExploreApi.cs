...
namespace SoundCloud.API.Client.Subresources
{
    public class ExploreApi : IExploreApi
    {
        ...
        public async Task<SCTrack[]> GetTracks(SCExploreCategory category, int offset = 0, int limit = 10)
        {
            paginationValidator.Validate(offset, limit);

            var parameters = new Dictionary<string, object>();

            var tracks = await soundCloudRawClient.Request<ExploreTrackCollection>(prefix, category.GetName(), HttpMethod.Get, parameters: parameters.SetPagination(offset, limit), domain: Internal.Client.Helpers.Domain.ApiV2, responseType: string.Empty);
