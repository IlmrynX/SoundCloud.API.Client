...
namespace SoundCloud.API.Client.Subresources
{
    public class AuthApi : IAuthApi
    {
        ...
        public async Task<SCAccessToken> AuthorizeByPassword(string userName, string password)
        {
            var credentials = soundCloudRawClient.Credentials;
            var accessToken = await soundCloudRawClient.Request<AccessToken>(prefix, "token", HttpMethod.Post, new Dictionary<string, object>
            {
        ...
        public async Task<SCAccessToken> AuthorizeByCode(string code, string redirectUri)
        {
            var credentials = soundCloudRawClient.Credentials;
            var accessToken = await soundCloudRawClient.Request<AccessToken>(prefix, "token", HttpMethod.Post, new Dictionary<string, object>
            {
        ...
        public async Task<SCAccessToken> RefreshToken(string token)
        {
            var credentials = soundCloudRawClient.Credentials;
            var accessToken = await soundCloudRawClient.Request<AccessToken>(prefix, "token", HttpMethod.Post, new Dictionary<string, object>
            {
